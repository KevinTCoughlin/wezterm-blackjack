...
local function bisearch(codepoint, ranges)
    local low = 1
    local high = #ranges
...
local function decode_next_utf8(text, index)
    ...
    if b1 <= 0xF4 then
        ...
        local codepoint = (b1 - 0xF0) * 0x40000 + (b2 - 0x80) * 0x1000 + (b3 - 0x80) * 0x40 + (b4 - 0x80)
...
function M.wcwidth(codepoint)
    ...
    if bisearch(codepoint, COMBINING_RANGES) then
        return 0
    end
    ...
    if codepoint >= 0x1100 and (
        ...
        or (codepoint >= 0x20000 and codepoint <= 0x2FFFD)
        or (codepoint >= 0x30000 and codepoint <= 0x3FFFD)
    ) then
        return 2
    end
...
function M.wcswidth(text)
    ...
    while index <= bytes do
        local codepoint, next_index = decode_next_utf8(text, index)
        if not codepoint then
            total = total + 1
            index = next_index
        else
            local width = M.wcwidth(codepoint)
            if width > 0 then
                total = total + width
            end
            index = next_index
...
function M.truncate(text, max_width)
    ...
    while index <= bytes do
        ...
        local char_width = 1
        if codepoint then
            char_width = M.wcwidth(codepoint)
